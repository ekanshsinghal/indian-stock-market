...
export function* fetchSearchListSaga(action) {
	yield put(actions.stocksSearchPending());
	try {
		const response = yield axios.post(ACTION_URLS.SEARCH + action.payload);
		yield put(actions.stocksSearchSuccess(response.data));
	} catch (error) {
		if(String(error) === 'Error: Request failed with status code 422')
			yield put(actions.logoutUser());
		yield put(actions.stocksSearchFail(error));
	}
}